...
const ImageCarousel = ({ isHero = false }: { isHero?: boolean }) => {
  ...
  const images: CarouselImage[] = [
    ...
    {
      url: "/WestlakeLiquorBroomfieldVodka1 - Edited.jpg",
  ...
  <section className={isHero ? "relative" : "py-16 bg-gray-900"}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Hero text block stays the same */}
      {isHero && (
        <div className="text-center mb-4 sm:mb-6 px-2 sm:px-6 lg:px-8 z-20 relative">
      ...
      <div className={`relative ${isHero ? 'w-full' : 'max-w-4xl mx-auto'}`}>
        ...
        <div className={`flex justify-center space-x-2 ${isHero ? 'absolute bottom-4 left-1/2 transform -translate-x-1/2 z-50' : 'mt-6 z-50'}`}>
          {images.map((_, index) => (
            <button
              key={index}
